...
$stdout = Join-Path $env:TEMP ("anomaly-log-hunter-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("anomaly-log-hunter-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5360"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/anomaly-lane"; File = "02-anomaly-lane-proof.png"; Size = "1600,1380" },
        @{ Url = "http://127.0.0.1:$port/source-patterns"; File = "03-source-patterns-proof.png"; Size = "1600,1260" },
